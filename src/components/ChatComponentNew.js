...
function ChatComponentNew() {
  ...
  //console.log('CurrentChatMessages =>', msgs)
  function search(nameKey, allMsgs){
    for (let i=0; i < allMsgs.length; i++) {
        if (allMsgs[i].id === nameKey) {
            console.log(allMsgs[i])
            currentChatMessages.push(allMsgs[i]);
        }
    }
}
...
search(currentChat._id, messages);
